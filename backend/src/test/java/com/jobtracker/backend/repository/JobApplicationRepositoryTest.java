...
class JobApplicationRepositoryTest {
	...
	void shouldSaveAndRetrieveJobApplication() {
		// Arrange
		JobApplication application = JobApplication.builder().company("Google").title("Software Engineer")
				.stage(ApplicationStage.APPLIED).notes("Excited about this opportunity!").appliedDate(LocalDate.now())
				.build();
	...
	void shouldUpdateJobApplication() {
		// Arrange
		JobApplication application = JobApplication.builder().company("Microsoft").title("Backend Developer")
				.stage(ApplicationStage.APPLIED).appliedDate(LocalDate.now()).build();
		JobApplication savedApplication = jobApplicationRepository.save(application);
	...
	void shouldDeleteJobApplication() {
		// Arrange
		JobApplication application = JobApplication.builder().company("Amazon").title("Cloud Architect")
				.stage(ApplicationStage.APPLIED).appliedDate(LocalDate.now()).build();
		JobApplication savedApplication = jobApplicationRepository.save(application);
	...
	void shouldFindAllJobApplications() {
		...
		JobApplication app1 = JobApplication.builder().company("Netflix").title("Data Engineer")
				.stage(ApplicationStage.APPLIED).appliedDate(LocalDate.now()).build();
		JobApplication app2 = JobApplication.builder().company("Apple").title("iOS Developer")
				.stage(ApplicationStage.OFFER).appliedDate(LocalDate.now()).build();
	...
	void shouldFindJobApplicationById() {
		// Arrange
		JobApplication application = JobApplication.builder().company("Meta").title("Frontend Engineer")
				.stage(ApplicationStage.APPLIED).appliedDate(LocalDate.now()).build();
		JobApplication savedApplication = jobApplicationRepository.save(application);
